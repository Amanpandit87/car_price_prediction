...
st.markdown("<div style='background-color:#219C90; border-radius:50px; align-items:center; justify-content: center;'><h1 style='text-align:center; color:white;'>Car Price Prediction</h1></div>",unsafe_allow_html=True)
...
with col2:
    ...
    kms_driven = st.slider("Enter the KMS Driven of Your Car",2000,140000,1000)

    col3, col4 = st.columns(2)

    with col3:
        fuel_type = st.selectbox("Select the Fuel Type",options=df["Fuel Type"].unique(),index=None)
    with col4:
        suspension_type = st.selectbox(label="Select the Suspension Type",options=df["Suspension"].unique(),index=None)
...
if pred:
    if any([car_model is None, kms_driven is None, fuel_type is None, suspension_type is None]):
        ...
        if prediction < 0:
            ...
            st.success(f"Predicted Price of Your Car is : ₹{int(prediction)}", icon="✅")
            # Using HTML to create a hyperlink
            st.markdown('<a href="/temp_rec" target="_self">From suggestion of more cars</a>', unsafe_allow_html=True)
